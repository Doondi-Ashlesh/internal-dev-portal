...
async function runSeed(options = {}) {
  ...
  try {
    const result = await seedDemoData(prisma, options);
    console.log(`Database seeded for ${result.workspace.name}. Workspace members: ${result.membersCount}.`);
  } catch (error) {
    console.error(error);
    process.exitCode = 1;
  } finally {
